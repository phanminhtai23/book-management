                ...
                <table class="table">
                    ...
                    <tbody>
                        <tr v-for="order in orders">
                            <td>
                                <img
                                    v-bind:src="order.sach.thumbnail"
                                    v-bind:alt="order.sach.bookName"
                                />
                            </td>
                            <td>{{ order.sach.TenSach }}</td>
                            <td>{{ order.theoDoiMuonSach.SoLuong }}</td>
                            <td>
                                <span
                                    v-if="
                                        order.theoDoiMuonSach.createdAt ===
                                        order.theoDoiMuonSach.updatedAt
                                    "
                                    >{{
                                        formatDate(order.theoDoiMuonSach.createdAt)
                                    }}</span
                                ><span v-else>{{
                                    formatDate(order.theoDoiMuonSach.updatedAt)
                                }}</span>
                            </td>
                        </tr>
                    </tbody>
    ...
    methods: {
        giveOrder(MaDocGia, bookId) {
            this.$request
                ...
                .then((res) => {
                    this.getAll(res.data.MaDocGia);
                    console.log(res.data);
                });
        },
        getAll(MaDocGia) {
            this.$request
                .get(`http://localhost:5268/api/order/${MaDocGia}`)
                .then((res) => {
                    if (res.data.success) {
                        this.orders = res.data.orders;
                        console.log("ne", this.orders);
                    }
                });
        },
        formatDate(value) {
            ...
            this.$request
                ...
                .then((res) => {
                    if (res.data.success) {
                        ...
                        this.getAll(this.myUser.MaDocGia);
                    }
                });
        },
        showUser() {
