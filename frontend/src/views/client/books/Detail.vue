                ...
                <div class="product-detail">
                    <div class="container my-5">
                        <div class="row">
                            ...
                            <div class="col-1"></div>
                            <div class="col-6">
                                <h2 class="inner-title">{{ book.TenSach }}</h2>
                                ...
                                <div class="inner-NamXuatBan">
                                    Năm xuất bản: <b>{{ book.NamXuatBan }}</b>
                                </div>
                                <div
                                    class="inner-SoQuyen mb-2"
                                    v-if="book.SoQuyen >= 1"
                                >
                                    Còn lại <span>{{ book.SoQuyen }}</span> sản
                                    phẩm
                                </div>
...
export default {
    ...
    methods: {
        getBook(slugBook) {
            console.log("slug", slugBook);
            this.$request
                .get(`http://localhost:5268/api/books/detail/${slugBook}`)
                .then((res) => {
                    if (res.data.success) {
                        console.log("backend trả ve,", res.data);
                        this.book = res.data.book;
                        this.publisher = res.data.NhaXuatBan;
        ...
        orderBook() {
            const tokenUser = this.$cookies.get("tokenUser");
            if (tokenUser) {
                this.order.MaDocGia = this.myUser.MaDocGia;
                this.order.MaSach = this.book.MaSach;
                this.$request
                    .post("http://localhost:5268/api/order/create", this.order)
                    .then((res) => {
                        if (res.data.success === true) {
                            this.getBook(this.$route.params.slugBook);
                            this.order.SoLuong = "1";
